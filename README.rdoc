...
AridCache supports pagination using WillPaginate.  The IDs from the cache are paginated in memory and only that page is selected from the database - directly from the target table, which is extremely fast.  Another advantage is that we already have the size of the collection in the cache so not query is required to set the <tt>:total_entries</tt> on the <tt>WillPaginate::Collection</tt>.
...
(Note that because the IDs are paginated in memory you cannot change the order of the collection while paginating.)

=== Limit & Offset

You can also use the <tt>:limit</tt> and <tt>:offset</tt> options.  These work slightly differently to pagination in that the IDs are not limited or offset in memory, but rather these options are applied to the whole collection in a regular call to <tt>find</tt>.

The thinking here is that we often want to get a subset of a cached collection, and this often requires applying some kind of (different) ordering to the collection as a whole.  For example, we may have cached a genre's tracks and now we want the top 10 tracks.

  genre.cached_tracks(:order => 'name ASC')
